...
/*
 * I'm going to strip away some of the things on chrome I don't need.
 * make some lock buttons as it is a pain sometimes.
 * make a better nav system
 */
 ...
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |explode |      |      |      |      |      | mute |           | undo | cut  | copy |paste |      |      |        | 
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * | caps   |   Q  |   W  |   F  |   P  |   G  | ctl  |           | ctl  |   J  |   L  |   U  |   Y  |   ;  |  del   |
 * |--------+------+------+------+------+------| left |           | rght |------+------+------+------+------+--------|
 * |        |   A  |  R   | S/BKT| T/NUM|   D  |------|           |------|   H  | N/vim|E/word|   I  |   O  |        |
 * |--------+------+------+------+------+------|LGui  |           | RGui |------+------+------+------+------+--------|
 * | Shift  |Z/Ctrl|   X  |   C  |   V  |   B  |      |           |      |   K  |   M  |   ,  |   .  |//Ctrl| RShift|
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |      | Left | Right|                                       |  Down  | Up |      |      | TG(4)|
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |CTRL/Esc| Alt|       | sft Alt  |Alt|
 *                                 ,------|------|------|       |------+--------+------.
 *                                 |      |      | Home |       | PgUp |        |      |
...
[BASE] = LAYOUT_ergodox(  // layer 0 : default
        // left hand
        LCTL(KC_UP),   KC_NO,        KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC__MUTE,
        KC_CAPS,       KC_Q,         KC_W,   KC_F,   KC_P,   KC_G,   LALT(LCTL(KC_LEFT)),
        KC_NO,       KC_A,         KC_R,   LT(PMM_BRACKETS, KC_S),   LT(PMM_NUMBER, KC_T),   KC_D,
        ...
        // right hand
             LCTL(KC_Z), LCTL(KC_X), LCTL(KC_C), LCTL(KC_V), KC_NO, KC_NO,            KC_NO,
             LALT(LCTL(KC_RGHT)),    KC_J,   KC_L,   KC_U,   KC_Y,   KC_SCLN,          KC_DELT,
                          KC_H,   LT(PMM_VIM, KC_N),   LT(PMM_WORDS, KC_E),   KC_I,   KC_O,   KC_NO,
             KC_RGUI,KC_K,   KC_M,   KC_COMM,      KC_DOT,  CTL_T(KC_SLSH),   KC_RSFT,
                                  KC_DOWN,  KC_UP, KC_NO, KC_NO, TG(PMM_NUMBER),
             LSFT( KC_RALT), KC_LALT,
             KC_PGUP,
...
[PMM_SYMB] = LAYOUT_ergodox(
       ...
       KC_TRNS,KC_EQL,KC_PPLS,KC_MINS,KC_UNDS,KC_TRNS,
...
[PMM_VIM] = LAYOUT_ergodox(
       // left hand
       KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS, KC_TRNS,KC_TRNS,KC_TRNS,
       KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS, LCTL(KC_UP), KC_TRNS,KC_TRNS,
...
[PMM_NUMBER] = LAYOUT_ergodox(
       ...
       // right hand
       KC_TRNS, KC_TRNS,   KC_TRNS, KC_TRNS, KC_TRNS,  KC_TRNS,  KC_TRNS,
       KC_TRNS, KC_PPLS,   KC_7,    KC_8,    KC_9,     KC_ASTR,  KC_TRNS,
...
[PMM_WORDS] = LAYOUT_ergodox(
       // left hand
       KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,
       KC_TRNS,KC_TRNS,KC_TRNS,M(5),M(4),KC_TRNS,KC_TRNS,
       KC_TRNS,KC_TRNS,M(8),M(7),M(6),KC_TRNS,
       KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,
       KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,KC_TRNS,
                                       KC_TRNS,KC_TRNS,
                                               KC_TRNS,
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  // MACRODOWN only works in this function
      switch(id) {
        ...
        case 6:
          if (record->event.pressed) {
            return MACRO( T(T), T(H), T(E), END  ); // 
          }
          break;
        case 7:
          if (record->event.pressed) {
            return MACRO( T(A), T(N), T(D), END  ); // 
          }
          break;
        case 8:
          if (record->event.pressed) {
            return MACRO( T(I), T(O), T(N), END  ); // 
          }
          break;
        /*
        case 9:
          if (record->event.pressed) {
            return MACRO( T(P), T(M), T(C), T(L), T(O), T(U), T(G), T(H), T(L), T(I), T(N), T(2), T(@), T(B), T(L), T(O), T(O), T(M), T(B), T(E), T(R), T(G), T(N), T(E), T(T), END  ); // 
          }
          break;
          */
      }
